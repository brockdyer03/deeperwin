...
from deeperwin.utils.utils import (
    ...
    get_from_devices,
    get_next_geometry_index,
    replicate_across_processes,
    pmap,
)
...
def optimize_wavefunction(
    ...
    cache_func_pmapped = pmap(cache_func, static_broadcasted_argnums=(1, 2))
    ...
    get_local_energy = build_local_energy(
        ...
        opt_config.forward_lap,
        opt_config.max_batch_size,
    )
    value_and_grad_func = build_value_and_grad_func(
        log_psi_squared, get_local_energy, opt_config.clipping, is_complex, kfac_register_complex
    )
    ...
    opt_state = initial_opt_state or optimizer.init(
        params=params,
        rng=rng_opt,
    ...
    for n_epoch in range(opt_config.n_epochs_prev, opt_config.n_epochs_prev + opt_config.n_epochs + 1):
        ...
        wf_logger.log_step(
            metrics, E_ref=phys_config.E_ref, mcmc_state=mcmc_state_merged, opt_stats=get_from_devices(stats)
        )
        ...
        if opt_config.stop_on_nan:
            ...
            if not np.isfinite(E):
                LOGGER.warning(
                    f"opt epoch {n_epoch:5d}: Hit non-finite optimization energy opt_E_mean={E}. Dumping checkpoint."
                )
                params_merged, fixed_params_merged, opt_state_merged, clipping_state_merged = get_from_devices(
                    (params, fixed_params, opt_state, clipping_state)
                )
...
def optimize_shared_wavefunction(
    ...
    for idx, g in enumerate(geometries_data_stores):
        ...
        g.mcmc_state = MCMCState.resize_or_init(
            g.mcmc_state, opt_config.mcmc, g.physical_config, jax.random.PRNGKey(rng_seed + idx)
        )
        ...
        g.mcmc_state, g.fixed_params = run_mcmc_with_cache(
            log_psi_squared, cache_func_pmapped, mcmc, params, g.spin_state, g.mcmc_state, g.fixed_params, mode="burnin"
        )
    ...
    for n_epoch in range(opt_config.n_epochs_prev, opt_config.n_epochs_prev + opt_config.n_epochs + 1):
        if is_checkpoint_required(n_epoch, opt_config.checkpoints):
            ...
            for idx_geom, g in enumerate(geometries_data_stores):
                ...
                if jax.process_index() == 0:
                    g.wavefunction_logger.loggers.log_checkpoint(
                        n_epoch,
                        params_merged,
                        fixed_params,
                        g.mcmc_state,
        ...
        if n_epoch in eval_checkpoints:
            ...
            for idx_geom, g in enumerate(geometries_data_stores):
                ...
                evaluate_wavefunction(
                    ...
                    g.n_opt_epochs,
                    dict(opt_n_epoch=n_epoch, geom_id=idx_geom),
                )
        if n_epoch == (opt_config.n_epochs_prev + opt_config.n_epochs):
            break
        ...
        params, opt_state, g.clipping_state, stats = optimizer.step(
            params=params,
            state=opt_state,
        ...
        ema_params = jax.tree_map(
            lambda old, new: opt_config.params_ema_factor * old + (1 - opt_config.params_ema_factor) * new,
            ema_params,
            params,
        )
        ...
        g.wavefunction_logger.log_step(
        ...
        if opt_config.stop_on_nan:
            ...
            if not np.isfinite(E):
                ...
                params_merged, fixed_params_merged, opt_state_merged, clipping_state_merged, ema_params_merged = (
                    get_from_devices((params, g.fixed_params, opt_state, g.clipping_state, ema_params))
                )
                g.wavefunction_logger.loggers.log_checkpoint(
                    n_epoch,
                    params_merged,
