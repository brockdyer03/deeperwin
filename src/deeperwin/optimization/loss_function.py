...
def _update_clipping_state(E, clipping_config: ClippingConfig):
    if jnp.iscomplexobj(E):
        ...
        if clipping_config.clip_imag_around_0:
            center_imag = 0.0
        return center_real + 1.0j * center_imag, width_real + 1.0j * width_imag
    else:
    ...
    if name == "hard":
        ...
        def _clip(E, center, width):
            if not jnp.iscomplexobj(E):
                ...
                return center + deviation_real + 1.0j * deviation_imag
...
def build_value_and_grad_func(
    log_psi_sqr_func, get_local_energy, clipping_config: ClippingConfig, is_complex=False, kfac_register_complex=False
):
    """
