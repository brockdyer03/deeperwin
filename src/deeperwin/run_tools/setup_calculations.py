...
def setup_calculations(args: list[str]):
    ...
    for param_tuple in args.parameter:
        ...
        if param_name in all_config_changes:
            raise ValueError(
                f"Each parameter may only be supplied once (but with multiple values if required). Duplicate parameter: {param_name}"
            )
    ...
    for exp_dir, exp_config_dict, exp_config in zip(experiment_dirs, experiment_config_dicts, experiment_configs):
        ...
        if (n_molecules > 1) and exp_config.optimization.shared_optimization:
            job_dirs, job_config_dicts, command = prepare_mulitple_geometries_on_single_job_shared(
                args, exp_dir, exp_config_dict
            )
        elif (n_molecules > 1) and not exp_config.optimization.shared_optimization:
            job_dirs, job_config_dicts, command = prepare_multiple_geometries_on_multiple_jobs(
                args, exp_dir, exp_config_dict
            )
        elif wandb_sweep:
            ...
            if "physical" in exp_config_dict:
                exp_config_dict["physical"] = exp_config.physical.create_geometry_list(
                    exp_config_dict["physical"].get("changes")
                )[0].dict()
