...
from deeperwin.model.definitions import Embeddings, WavefunctionDefinition
...
from deeperwin.orbitals import (
    get_atomic_orbital_descriptors,
...
class JastrowFactor(hk.Module):
    ...
    def __call__(self, embeddings: Embeddings, n_up: int):
        if self.config.differentiate_spins:
            jastrow_up = MLP(
                self.config.n_hidden + [1], self.mlp_config, linear_out=True, output_bias=False, name="up"
            )(embeddings.el[..., :n_up, :])
            jastrow_dn = MLP(
                self.config.n_hidden + [1], self.mlp_config, linear_out=True, output_bias=False, name="dn"
            )(embeddings.el[..., n_up:, :])
...
class Wavefunction(hk.Module):
    def __init__(self, config: ModelConfig, wavefunction_definition: WavefunctionDefinition, name: str = "wf") -> None:
        """
        Args:
        ...
        self.input_preprocessor = InputPreprocessor(
            config=config.features, mlp_config=config.mlp, wavefunction_definition=wavefunction_definition, name="input"
        )
        ...
        self.orb_net = OrbitalNet(
            self.config.orbitals,
            self.config.mlp,
    ...
    def _calculate_embedding(self, diff_dist, features, n_up):
        ...
        if self.config.embedding.name == "gnn":
            return GNNEmbedding(self.config.embedding, self.config.mlp)(diff_dist, features, n_up)
        elif self.config.embedding.name == "transformer":
            return TransformerEmbedding(self.config.embedding, self.config.mlp)(features, n_up)
    ...
    def _calculate_cache(self, n_up: int, n_dn: int, r, R, Z, fixed_params: dict | None = None):
        ...
        if (fixed_params is not None) and fixed_params.get("periodic") is not None:
            diff_ion_ion, dist_ion_ion = get_periodic_distance_matrix(
                R, fixed_params["periodic"].lattice, fixed_params["periodic"].rec / (2 * np.pi)
            )
        else:
            diff_ion_ion, dist_ion_ion = get_distance_matrix(R)

        if (
            self.config.orbitals.transferable_atomic_orbitals
            and self.config.orbitals.transferable_atomic_orbitals.name == "taos"
        ):
            cache["taos"] = dict()
            if self.config.embedding.name == "axial_transformer":
                ...
                attention_value_dim = (
                    self.config.embedding.el_transformer.attention_value_dim
                    or self.config.embedding.el_transformer.attention_dim
                )
                emb_dim = (
                    self.config.embedding.el_transformer.attention_output_dim
                    or attention_value_dim * self.config.embedding.el_transformer.n_heads
                )
    ...
    def _el_el_cusp(self, el_el_dist, n_up):
        ...
        if el_el_dist.shape[-1] == el_el_dist.shape[-2]:
            ...
            dist_same = jnp.concatenate(
                [el_el_dist[..., :n_up, :n_up].reshape(flat_shape), el_el_dist[..., n_up:, n_up:].reshape(flat_shape)],
                axis=-1,
            )
            dist_diff = jnp.concatenate(
                [el_el_dist[..., :n_up, n_up:].reshape(flat_shape), el_el_dist[..., n_up:, :n_up].reshape(flat_shape)],
                axis=-1,
            )
        else:
            dist_same = jnp.concatenate(
                [
                    el_el_dist[..., :n_up, : n_up - 1].reshape(flat_shape),
                    el_el_dist[..., n_up:, n_up:].reshape(flat_shape),
                ],
                axis=-1,
            )
            dist_diff = jnp.concatenate(
    ...
    def init_for_multitransform(self):
        return self.__call__, (
            self.__call__,
            self.get_slater_matrices,
            self._calculate_embedding,
            self._calculate_orbitals,
...
def build_log_psi_squared(
    ...
    fixed_params = fixed_params or init_model_fixed_params(config, _phys_config, baseline_config)
...
def construct_wavefunction_definition(
    ...
    return WavefunctionDefinition(Z_max=Z_max, Z_min=Z_min)
...
def check_orbital_intialization(
    ...
    if (
        ...
        and config.orbitals.envelope_orbitals.initialization == "analytical"
    ):
        weights, alphas = get_envelope_exponents_from_atomic_orbitals(
            physical_config,
            pad_full_det=(
                (config.orbitals.determinant_schema == "full_det")
                or (config.orbitals.determinant_schema == "restricted_closed_shell")
            ),
        )
        ...
        params["wf/orbitals/envelope_orbitals"]["weights_dn"] = jnp.tile(
            weights[0], [1, config.orbitals.n_determinants]
        )
        params["wf/orbitals/envelope_orbitals"]["weights_up"] = jnp.tile(
            weights[1], [1, config.orbitals.n_determinants]
        )
    return params
...
def init_model_fixed_params(
    ...
    if tao_config:
        ...
        nb_orbitals_per_Z = get_n_basis_per_Z(
            baseline_config.basis_set, tuple(atom_types), is_periodic, baseline_config.pyscf_options.exp_to_discard
        )
        orbital_features = get_atomic_orbital_descriptors(
            orbital_params,
            R,
            Z,
        ...
        if is_periodic:
            k_twist_frac = (
                np.array(physical_config.periodic.k_twist)
                if physical_config.periodic.k_twist is not None
                else np.zeros(3)
            )
            twist_features = []
            ...
            if tao_config.twist_encoding is not None and "periodic" in tao_config.twist_encoding:
                ...
                periodic_twist = np.concatenate(
                    [jnp.sin(R @ k_twist_frac)[..., None], jnp.cos(R @ k_twist_frac)[..., None]], axis=-1
                )[..., None, :]
        ...
        fixed_params["transferable_atomic_orbitals"] = dict(
            orbitals=orbital_params, features=orbital_features, features_ion_phisnet=nuc_features
        )
