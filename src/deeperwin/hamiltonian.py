...
from folx import forward_laplacian, batched_vmap
...
def real_space_ewald(diff, lat_vectors, gamma):
    ...
    """
    displacements = jnp.linalg.norm(diff - lat_vectors, axis=-1)  # |r - R|
    return jnp.sum(jax.scipy.special.erfc(gamma**0.5 * displacements) / displacements)
...
def ewald_sum(diff, volume, rec_vectors, rec_vec_weights, gamma, lat_vectors):
    ...
    """
    return (
        real_space_ewald(diff, lat_vectors, gamma)
        + recp_space_ewald(diff, rec_vectors, rec_vec_weights)
        - jnp.pi / (volume * gamma)
    )


def get_ewald_el_ion_potential_energy(
    diff_el_ion, Z, volume, rec_vectors, rec_vec_weights, gamma, lat_vectors, madelung_const
):
...
def get_ewald_el_el_potential_energy(
    diff_el_el, volume, rec_vectors, rec_vectors_weights, gamma, lat_vectors, madelung_const, include_heg_background
):
...
def get_ewald_ion_ion_potential_energy(
    R, Z, volume, rec_vectors, rec_vectors_weights, gamma, lat_vectors, madelung_const
):
    # Atom-atom potential
    n_ions = R.shape[0]
    if n_ions > 1:
...
def get_ewald_potential_energy(r, R, Z, lattice_params: LatticeParams, include_heg_background):
    ...
    diff_el_ion_first_cell = project_into_first_unit_cell(
        diff_el_ion, lattice_params.lattice, lattice_params.rec / (2 * np.pi)
    )
    diff_el_el_first_cell = project_into_first_unit_cell(
        diff_el_el, lattice_params.lattice, lattice_params.rec / (2 * np.pi)
    )

    return jnp.real(
        get_ewald_el_ion_potential_energy(
            diff_el_ion_first_cell,
            Z,
...
def get_kinetic_energy(
    log_psi_squared_and_phase, trainable_params, spin_state, r, R, Z, fixed_params, complex_wf, forward_lap=True
):
    """This code here is strongly inspired by the implementation of FermiNet (Copyright 2020 DeepMind Technologies Limited.)"""
    ...
    if forward_lap:
        if not complex_wf:
            curry_log_psi_squared = lambda r: log_psi_squared(trainable_params, *spin_state, r, R, Z, fixed_params)
            fwd_log_psi_squared = forward_laplacian(curry_log_psi_squared)
            log_psi_squared_results = fwd_log_psi_squared(r)
            log_psi_squared_grad, log_psi_squared_laplacian = (
                log_psi_squared_results.jacobian.dense_array,
                log_psi_squared_results.laplacian,
            )

            energy = -0.5 * (0.5 * log_psi_squared_laplacian + 0.25 * jnp.sum(log_psi_squared_grad**2))
        else:
            curry = lambda r: log_psi_squared_and_phase(trainable_params, *spin_state, r, R, Z, fixed_params)
            fwd_log_psi_squared_and_phase = forward_laplacian(curry)
            fwd_lap_results = fwd_log_psi_squared_and_phase(r)
            log_psi_squared_grad, log_psi_squared_laplacian = (
                fwd_lap_results[1].jacobian.dense_array,
                fwd_lap_results[1].laplacian,
            )
            phase_grad, phase_laplacian = fwd_lap_results[0].jacobian.dense_array, fwd_lap_results[0].laplacian

            energy = -0.5 * (
                0.5 * log_psi_squared_laplacian
                + 0.25 * jnp.sum(log_psi_squared_grad**2)
                + 1.0j * jnp.sum(log_psi_squared_grad * phase_grad)
                - jnp.sum(phase_grad**2)
                + 1.0j * phase_laplacian
            )
    else:
        ...
        # Calculate gradient and jvp_func(v)=J_g*v (i.e. the Jacobian vector product of the gradient, aka the Hessian vector product of log(psi^2)
        ...
        if complex_wf:
            ...
            def grad_phase_func(r):
                return jax.grad(phase_log_psi_squared, argnums=3)(
                    trainable_params, *spin_state, r.reshape([-1, 3]), R, Z, fixed_params
                ).flatten()
            ...
            laplacian -= 1.0j * jnp.sum(0.5 * grad_value * grad_phase_value)  # TODO double check

            def _loop_body(i, accumulator):
                return accumulator + jvp_func(eye[i])[i] + 2 * 1.0j * jvp_phase_func(eye[i])[i]
        else:

            def _loop_body(i, accumulator):
                # jvp_func(eye[i])[i] = H[i][i] of log(psi^2)
...
def build_local_energy(
    log_psi_squared,
    is_complex=False,
    is_periodic=False,
    include_heg_background=False,
    forward_lap=False,
    max_batch_size=64,
):
    @functools.partial(batched_vmap, in_axes=(None, None, 0, None, None, None), max_batch_size=max_batch_size)
    def get_local_energy(trainable_params, spin_state, r, R, Z, fixed_params):
        E_kin = get_kinetic_energy(
            log_psi_squared, trainable_params, spin_state, r, R, Z, fixed_params, is_complex, forward_lap=forward_lap
        )  # adapt directly the function
        if is_periodic:
...
def _calculate_ion_ion_forces(R, Z):
    ...
    forces = (
        diff * Z_matrix[..., jnp.newaxis] / (dist + EPSILON) ** 3
    )  # tiny EPS ensures that the diagonal (self-interaction) has 0 contribution
    return jnp.sum(forces, axis=1)


def calculate_forces(
    log_sqr_func, params, r, R, Z, fixed_params, log_psi_sqr, config: ForceEvaluationConfig, poly_coeffs=None
):
    """
